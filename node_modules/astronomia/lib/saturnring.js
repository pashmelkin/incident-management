 ...
 */
function ring(jde, earth, saturn) {
...
function cl(jde, earth, saturn) {
  ...
  var f1 = function f1() {
    ...
    i = _base["default"].horner(T, 28.075216 * p, -0.012998 * p, 0.000004 * p);
    Ω = _base["default"].horner(T, 169.50847 * p, 1.394681 * p, 0.000412 * p); // Step 2.0
    ...
    var sb0 = Math.sin(b0); // Steps 3, 4.0
    ...
    var f = function f() {
      ...
      var fk5 = _planetposition["default"].toFK5(saturnPos.lon, saturnPos.lat, jde);
    ...
    λ = Math.atan2(y, x);
    β = Math.atan(z / Math.hypot(x, y)); // First part of step 6.0
    ...
    var bʹ = b - 0.000764 * p * Math.cos(l - N) / r; // Setup for steps 8, 9.0

    sbʹ = Math.sin(bʹ);
    cbʹ = Math.cos(bʹ);
    slʹΩ = Math.sin(lʹ - Ω);
    clʹΩ = Math.cos(lʹ - Ω); // Step 9.0

    var _base$sincos7 = _base["default"].sincos(λ - Ω),
  ...
  var f2 = function f2() {
    ...
    var ε = _nutation["default"].meanObliquity(jde) + Δε; // Step 11.0

    var λ0 = Ω - Math.PI / 2;
    var β0 = Math.PI / 2 - i; // Step 12.0

    var _base$sincos9 = _base["default"].sincos(l0 - λ),
        _base$sincos10 = _slicedToArray(_base$sincos9, 2),
        sl0λ = _base$sincos10[0],
        cl0λ = _base$sincos10[1];

    λ += 0.005693 * p * cl0λ / cβ;
    β += 0.005693 * p * sl0λ * sβ; // Step 13.0

    λ0 += Δψ;
    λ += Δψ; // Step 14.0

    var eq = new _coord["default"].Ecliptic(λ0, β0).toEquatorial(ε);
    ...
    var _base$sincos11 = _base["default"].sincos(δ0),
    ...
    var _base$sincos15 = _base["default"].sincos(α0 - α),
