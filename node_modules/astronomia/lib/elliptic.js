 ...
 */
function position(planet, earth, jde) {
  ...
  var _base$sincos = _base["default"].sincos(B0),
      _base$sincos2 = _slicedToArray(_base$sincos, 2),
      sB0 = _base$sincos2[0],
      cB0 = _base$sincos2[1];

  var _base$sincos3 = _base["default"].sincos(L0),
  ...
  function pos() {
    ...
    var _base$sincos7 = _base["default"].sincos(L),
  ...
  var τ = _base["default"].lightTime(Δ); // repeating with jde-τ
  ...
  var _apparent$eclipticAbe = _apparent["default"].eclipticAberration(λ, β, jde),
      _apparent$eclipticAbe2 = _slicedToArray(_apparent$eclipticAbe, 2),
      Δλ = _apparent$eclipticAbe2[0],
      Δβ = _apparent$eclipticAbe2[1];

  var fk5 = _planetposition["default"].toFK5(λ + Δλ, β + Δβ, jde);
  ...
  return new _coord["default"].Ecliptic(λ, β).toEquatorial(ε); // Meeus gives a formula for elongation but doesn't spell out how to
  ...
  */
  ...
  _createClass(Elements, [{
    ...
    value: function position(jde, earth) {
      ...
      // (α, δ, ψ float64) {
      // (33.6) p. 227
      var n = _base["default"].K / this.axis / Math.sqrt(this.axis);
      var sε = _base["default"].SOblJ2000;
      var cε = _base["default"].COblJ2000;

      var _base$sincos9 = _base["default"].sincos(this.node),
          _base$sincos10 = _slicedToArray(_base$sincos9, 2),
          sΩ = _base$sincos10[0],
          cΩ = _base$sincos10[1];

      var _base$sincos11 = _base["default"].sincos(this.inc),
          _base$sincos12 = _slicedToArray(_base$sincos11, 2),
          si = _base$sincos12[0],
          ci = _base$sincos12[1]; // (33.7) p. 228
      ...
      var R = cΩ * ci * sε + si * cε; // (33.8) p. 229
      ...
      var f = function f(jde) {
        ...
        try {
          E = _kepler["default"].kepler2b(_this.ecc, M, 15);
        } catch (e) {
          E = _kepler["default"].kepler3(_this.ecc, M);
        }

        var ν = _kepler["default"].trueAnomaly(E, _this.ecc);

        var r = _kepler["default"].radius(E, _this.ecc, _this.axis); // (33.9) p. 229
...
function astrometricJ2000(f, jde, earth) {
  // (f func(float64)  (x, y, z float64), jde float64, e *pp.V87Planet) (α, δ, ψ float64)
  var sol = _solarxyz["default"].positionJ2000(earth, jde);
  ...
  return new _base["default"].Coord(α, δ, undefined, ψ);
}
/**
